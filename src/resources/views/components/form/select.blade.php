<label {{ $attributes->merge(['class' => 'block']) }}>
    ...
    @endif
    <select class="focus:border-gray-500 focus:bg-white focus:ring-0 block w-full my-1 bg-muted-lightest border-transparent rounded-md @error($wireid) border-red-500 @enderror" @if ($wireid) wire:model.debounce.500ms="{{ $wireid }}" @endif>
        <option value="" hidden></option>
        @foreach ($options as $key => $value)
            <option value="{{ $key }}">{{ $value }}</option>
        @endforeach
    </select>
    @if ($description || @error($wireid))
