<div {{ $attributes->merge(['class' => 'w-full']) }} @if ($max) x-data="{ count: 0 }" x-init="count = $refs.inputfield.value.length" @endif>
    ...
    <div class="mt-1">
        <input type="{{ $inputType }}" class="focus:border-blue-500 block w-full placeholder-gray-300 border-gray-300 rounded read-only:bg-gray-100 @error($wireId) border-red-500 @enderror" @if ($placeholder) placeholder="{{ $placeholder }}"@endif @if ($max) maxlength="{{ $max }}" x-ref="inputfield" x-on:keyup="count = $refs.inputfield.value.length" @endif @if ($wireId) wire:model.lazy="{{ $wireId }}" @endif>
    </div>
    <div class="flex items-center justify-between">
        @if ($desc)
